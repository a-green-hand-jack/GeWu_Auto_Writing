...
本 skill 是写作流程的最后一道工序：正文定稿、组装之前用它清理 AI 痕迹。

多条规则需要先判断“原语体”再决定是否改写：第 7 条按体裁判断，第 4、5 条
看原文的标点习惯，第 8 条看原文是否已给出具体数据。若本领域指南或风格记录
给出了目标写法（如句长分布、标点习惯、用词偏好），以它为准：那是目标作者
的真实写法，不是 AI 痕迹。
...
触发标记：冒号前是提示语，用来引出总结、解释、定义或结论，包括“一句话总结：”“核心是：”“关键在于：”“原因如下：”“结论：”“本质上：”“换句话说：”。实测 AI 0.32/千字、人类 0.08，人类侧稳定偏低。

不改：引出人物直接原话（“他说：‘明天再谈。’”）；小标题里的冒号；列表项的冒号；正常句中冒号（“人擅长的是：判断业务逻辑”这类先总后分的自然写法）；网址、代码和机器字段。

改法：删掉提示语，直接写内容；或在原段落内改成完整句子。
