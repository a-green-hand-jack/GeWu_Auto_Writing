...
All paths in this section are relative to this skill's directory: the folder
containing this `SKILL.md` (for example `.agents/skills/proofreading`). Resolve
them against that directory, not the current working directory.

After writing the Markdown report, automatically convert it to PDF by running:

```bash
python <skill-dir>/scripts/generate_report_pdf.py <report.md>
```

Run this as the final step of report mode. If the script fails (missing `pandoc` or LaTeX), print the error and note that the Markdown report is still complete and readable. The LaTeX template is at `<skill-dir>/templates/report_latex.tex`.
