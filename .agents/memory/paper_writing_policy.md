...
## 「生产说明」要求已废除（2026-09-14）

旧设计要求每篇论文在附录前加一个未编号小节 "Production and authorship"，披露来源仓库、协作 agent、harness、模型与所做检查；理由是"诚实标注"。**这是错的**：① 没有任何期刊要求这个字段；② 它把工具与平台名写进了稿面，与"不出现 GeWu 黑话"直接冲突。

替代方案：**来源仓库按普通参考文献引用**（每仓库一条，多仓库就是多条，像引用 GitHub 仓库那样），全文其他地方不出现 harness/model/provider/host/agent/prompt/run/check 任何字样——已无例外（旧 constitution §7 的"唯一允许出现"条款一并删除）。

已同步改动：`constitution.md` §7 重写、`production.md` 装配步骤、`checks.md` §2 校验改为"仓库是否作为参考文献被引用"、`SKILL.md`、以及**全部启动器**（4–8 轮旧启动器已就地更新，git 历史保留原样）。已交付的第七轮稿件仍带该小节，由第九轮取代。


## 散文形态：规则存在，但门禁量错了东西（2026-09-15，第十一轮）

同事复核已交付稿件提出两点：① 有些 section 每段都以 `a.` / `b.` / `c.` 标签起首；② DISCUSSION AND OUTLOOK 被 A. / B. subsection 切成骨架。

- ② **已不存在**。逐轮实测（讨论/结论区含字母子节）：round1 = 18/30、round3 ≈ 19/30、round5 = 5/30、round10 = 0/30——它是当年强加章节骨架的残留，撤掉骨架后自愈。仍写入 `writing.md` 以防复发。
- ① **仍在**，而且**技能里早有这条规则**（`checks.md` §8 "a section that is a bullet list is an outline"），外加 `gewu-verify` 的 prose-shape 门禁——round10 却是全绿通过。根因：**两者都只统计 `\item` 密度**，而我们的稿子平均每篇只有 1.0 个 list 环境。LLM 只是换了实现方式：给每段贴标签（`\paragraph{}` / `\textbf{}` / 字母）。round10 实测 3280 段中 275 段标签起首（8.4%），最差正文文件是 `discussion.tex`：6 段里 5 段（01 / 08 / 30 三篇都如此）。
- 修法（最小增量，全部复用已有结构）：规则搬到**起草前必读**的 `writing.md` §9（"散文是默认单位" + 标签段落 tell + 讨论/展望/相关工作/结论通篇散文）；`checks.md` §8 改为按文件统计标签起首段落占比；`gewu-verify.prose_shape` 同口径，并**读 `main.tex` 判断哪些文件在 `\appendix` 之后**（附录豁免，避免误报）。
- 结果（round11，30/30）：标签起首段落 64/2942 = **2.2%**（round10 为 8.4%）；新门禁 **0 篇失败**（round10 为 5 篇）；讨论区 subsection 0 篇；其余 13 项门禁零回归。
- 顺带修掉门禁自身两个误报：豁免名单里的 `appendix` **不匹配复数 `appendices`**（改为递归跟随"只含 `\input` 的容器文件"，round11 论文 15 命中）；PDF 附录起始页检测把正文换行后的 `Appendix E.` 当成附录标题（改为只看页首 200 字符）。
- **教训**：门禁必须量**真实病灶**。规则写得再清楚，只要门禁量的是另一种特征，它就会一直"通过"——这比没有门禁更糟，因为它把缺陷变成了绿灯。

## 附录小标题与编号：已核查，决定保持现状（2026-09-15）

用户注意到附录里出现**带编号的小标题**（如 `Appendix A: …` 之下 `1.` `2.` `3.`）。核查结论：

- **这是期刊类文件的原生行为，不是排版错误**。用 TeX Live 里的真实类文件编译探测文档验证：revtex4-2/PRX 的附录中，字母在节标题里，内部 `\subsection` 印成裸 `1.`（每个附录重起）、`\subsubsection` 印成 `a.`；amsart 则印成 `A.1.` / `A.1.1.`。与正文层级（`A.` 子节、`1.` 子子节）不撞号。
- 分布（round11 交付件，30 篇）：**PRX `\subsection` 编号 14 篇**、amsart `\subsection*` 不编号 3 篇、Nature `\paragraph` 1 篇、无小标题 12 篇。最多的是 `26-uniform-csm`（5 个附录共 27 个编号小标题）与 `07-sun-aklt`（8 个附录共 26 个）。
- **为什么值得单独决策**：`checks.md` §8 主动豁免了附录（"附录可以合法分条"），因此附录的小标题层级与数量**没有任何门禁**——这正是"一篇 5 个附录 27 个编号小标题"能出现的原因。跨篇形态也不一致（编号/带星号/行内/无），这是我们的差异而非期刊差异。
- **决定：保持现状**（附录是核对材料，可导航性与可引用的稳定编号有价值；改它等于在期刊风格之上强加自家风格）。用户 2026-09-15 确认。
- 记录：**GitHub issue #3**（`a-green-hand-jack/GeWu_Auto_Writing`）。若日后要改，选项是"限深限量"或"禁止（附录只保留期刊自身节级别）"，需要改 `writing.md`（起草前必读）+ `checks.md` §8 与 `gewu-verify.prose_shape`（它已能判断哪些文件在 `\appendix` 之后），全量重跑约 2 小时或只重跑 14 篇 PRX。
